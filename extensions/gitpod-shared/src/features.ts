...
export function registerActiveNotifications(context: GitpodExtensionContext): void {
	updateActiveNotifications(context);
	context.subscriptions.push(vscode.window.onDidChangeWindowState(() => updateActiveNotifications(context)));
}

let toStopObservingActiveNotifiactions: vscode.Disposable | undefined;
function updateActiveNotifications(context: GitpodExtensionContext): void {
	if (!vscode.window.state.focused) {
		toStopObservingActiveNotifiactions?.dispose();
		toStopObservingActiveNotifiactions = undefined;
	} else if (!toStopObservingActiveNotifiactions) {
		toStopObservingActiveNotifiactions = observeActiveNotifications(context);
	}
}
